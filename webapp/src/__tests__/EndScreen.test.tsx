...
describe('Home', () => {
    ...
    test('shows wellcome message with username', () => {
        ...
        expect(screen.getByText(/bienvenida a tu menú principal, sara/i)).toBeInTheDocument()
    })
    ...
    test('navega a stats al pulsar Mis estadísticas', async () => {
        render(<Home username="sara" />)
    ...
    test('navega al ranking al pulsar Ranking', async () => {
    ...
    test('inicia partida vs bot llamando a /initmatch', async () => {
        global.fetch = vi.fn().mockResolvedValue({ ok: true, json: async () => ({}) } as Response)
        render(<Home username="sara" />)
    ...
    test('seleccionar dificultad fácil activa su botón', async () => {
        render(<Home username="sara" />)
    ...
    test('seleccionar estrategia en el select actualiza la selección', async () => {
        render(<Home username="sara" />)
